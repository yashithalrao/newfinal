...
const fetchPendingStats = async () => {
  try {
    const start = new Date(startDateFilter || new Date().toISOString().split("T")[0]);
    const end = new Date(endDateFilter || new Date().toISOString().split("T")[0]);
    
    // ✅ Calculate selected day count
    const msInDay = 1000 * 60 * 60 * 24;
    const filteredDayCount = Math.ceil((end - start) / msInDay) + 1;

    const dailyMax = 8;
    const maxHrsForFilter = dailyMax * filteredDayCount;
    ...
    // ✅ Apply overflow calculation
    const processedStats = res.data.map((entry) => {
      const normalHrs = Math.min(entry.pendingHrs, maxHrsForFilter);
      const extraHrs = Math.max(0, entry.pendingHrs - maxHrsForFilter);
      const freeTime = Math.max(0, maxHrsForFilter - entry.pendingHrs);
    ...
    setPendingStats(processedStats);
  } catch (err) {
    console.error("Pending stats fetch failed:", err);
  }
};
...
<div style={{ overflowX: 'auto' }}>
  <BarChart width={pendingStats.length * 100} height={300} data={pendingStats}>
    <CartesianGrid strokeDasharray="3 3" />
    <XAxis dataKey="person" />
    <YAxis />
    <Tooltip />
