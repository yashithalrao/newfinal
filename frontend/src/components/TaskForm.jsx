...
//const editableFields = Object.keys(initialTask);
//const editableFields = Object.keys(initialTask).filter(key => key !== 'hoursPerDay');
...
export default function TaskForm({ onTaskCreated, editTask, onTaskUpdated }) {
  ...
  //     editableFields.forEach(field => {
  //       //filtered[field] = editTask[field] || '';
  //       filtered[field] = editTask[field] !== undefined ? editTask[field] : '';

  //   console.log("EDIT TASK LOADED →", editTask);
...
useEffect(() => {
  if (editTask) {
    ...
    // Explicitly handle hoursPerDay even though it's not in editableFields
    filtered["hoursPerDay"] = editTask["hoursPerDay"] !== undefined ? editTask["hoursPerDay"] : '';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        slNo: Number(task.slNo),
        //plannedHrs: Number(task.plannedHrs),
        hoursPerDay: Number(task.hoursPerDay) || 0,
  ...
  return (
        ...
        {editableFields.map((key) => {
          ...
          return (
            ...
            <label style={{ fontWeight: 'bold', display: 'block', marginBottom: '5px' }}>
              {key === 'slNo' ? 'Sl No:' : key === 'personHandling' ? 'Assigned To:' : key.charAt(0).toUpperCase() + key.slice(1) + ':'}
            </label>
            ...
            <div key={key} style={{ marginBottom: '12px' }}>
              {key === 'priority' ? (
                ...
                <input
                  ...
                  name={key}
                  // value={task[key]?.slice(0, 10)}
                  value={task[key] ? formatDateTimeLocal(task[key]) : ''}
    ...
    cursor: isHalted ? 'not-allowed' : 'text'
          ...
          {task.start && task.end && (
  new Date(task.end).toDateString() !== new Date(task.start).toDateString() && (
    <div style={{ marginBottom: '12px' }}>
      <input
        type="number"
        name="hoursPerDay"
        ...
        onChange={handleChange}
        placeholder="How many hours a day are being allocated"
        style={input}
          required
  min={1}
  max={8}
      />
        ...
        {task.status === 'Halt' && (
          ...
          <div style={{ marginBottom: '12px' }}>
            <label style={{ fontSize: '14px' }}><b>Reason for hold</b></label>
            <input
              type="text"
              name="remarks"
              value={task.remarks}
